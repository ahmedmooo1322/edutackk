...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _load([AppScope? scope]) async {
    ...
    final result = await app.apiClient.me();
    if (!mounted) return;
    setState(() {
      _loading = false;
      if (result.ok) {
  ...
  Widget build(BuildContext context) {
    ...
    final name = user['name']?.toString() ?? context.tr('student');
            ...
            ? Center(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(context.tr('loadingAccount')),
                  ],
                ),
              )
            : RefreshIndicator(
                  ...
                  children: [
                    ...
                    if (_error.isEmpty) ...[
                      ...
                      _HomeCard(
                        ...
                        title: context.tr('levelRoom'),
                        subtitle: context.tr('levelRoomSubtitle'),
                        onTap: () => Navigator.pushNamed(context, '/level-room'),
                      ),
                      _HomeCard(
                        icon: Icons.search,
                        title: context.tr('searchStudents'),
                        subtitle: context.tr('searchStudentsSubtitle'),
                        onTap: () => Navigator.pushNamed(context, '/student-search'),
                      ),
                      _HomeCard(
                        icon: Icons.people_alt_outlined,
                        title: context.tr('friends'),
                        subtitle: context.tr('friendsSubtitle'),
