...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _load([AppScope? scope]) async {
    ...
    if (me.ok) user = me.data?['user'] as Map<String, dynamic>?;
    final isStudent = (user?['role'] ?? 'student') == 'student';
    final profile = isStudent ? await app.apiClient.studentProfile() : null;
    final sub = isStudent ? await app.apiClient.subscription() : null;
    if (!mounted) return;
    setState(() {
      _loading = false;
      if (me.ok) _user = user;
      if (profile?.ok == true) _student = profile?.data?['student'] as Map<String, dynamic>?;
      if (sub?.ok == true) _subscription = sub?.data;
      _error = [if (!me.ok) me.error, if (profile != null && !profile.ok) profile.error, if (sub != null && !sub.ok) sub.error].whereType<String>().join('\n');
    });
  }
  ...
  Future<void> _requestProfileEdit() async {
    ...
    final subjects = TextEditingController(text: '${(user['profile'] as Map?)?['subjects'] ?? ''}'.replaceAll('[', '').replaceAll(']', ''));
    final isTeacher = (user['role'] ?? 'student') == 'teacher';
    final ok = await showDialog<bool>(context: context, builder: (context) => AlertDialog(
      title: Text(context.tr('requestProfileEdit')),
      content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        if (isTeacher) TextField(controller: subjects, decoration: InputDecoration(labelText: context.tr('teacherSubjects'), helperText: context.tr('subjectsHelp'))),
        const SizedBox(height: 8),
        Text(context.tr('profileEditApprovalNote'), style: Theme.of(context).textTheme.bodySmall),
      ])),
      actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: Text(context.tr('cancel'))), FilledButton(onPressed: () => Navigator.pop(context, true), child: Text(context.tr('sendRequest')))],
    ));
    if (ok != true) return;
    final body = <String, dynamic>{
      ...
      if (isTeacher) 'subjects': subjects.text.split(',').map((e) => e.trim()).where((e) => e.isNotEmpty).toList(),
    };
    final res = await AppScope.of(context).apiClient.requestProfileChange(body);
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(res.ok ? context.tr('profileEditRequestSent') : (res.error ?? context.tr('connectionProblem')))));
  }
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  OutlinedButton.icon(onPressed: _requestProfileEdit, icon: const Icon(Icons.edit_outlined), label: Text(context.tr('requestProfileEdit'))),
                ],
              ),
            ),
          ),
          ...
          const SizedBox(height: 12),
          _InfoTile(label: (user['role'] ?? 'student') == 'teacher' ? context.tr('teacherNo') : context.tr('studentId'), value: '${user['id'] ?? student['user_id'] ?? '-'}'),
          ...
          if ((user['role'] ?? 'student') == 'teacher') _InfoTile(label: context.tr('teacherSubjects'), value: '${(user['profile'] as Map?)?['subjects'] ?? '-'}'),
          if ((user['role'] ?? 'student') == 'student') _InfoTile(label: context.tr('overallRate'), value: '${student['overall_rate'] ?? 0}'),
          if ((user['role'] ?? 'student') == 'student') _InfoTile(label: context.tr('questionsAsked'), value: '${student['total_questions_asked'] ?? 0}'),
        ],
      ),
    );
  }
