...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  Future<void> _loadUsers() async {
    ...
    final result = await AppScope.of(context).apiClient.adminUsers(q: _search.text);
    if (!mounted) return;
    setState(() {
      _loadingUsers = false;
      if (result.ok) {
  ...
  Future<void> _loadReports() async {
    final result = await AppScope.of(context).apiClient.adminReports();
    if (!mounted) return;
    if (result.ok) setState(() => _reports = _listFrom(result.data?['reports']));
  }

  Future<void> _loadDeletionRequests() async {
    final result = await AppScope.of(context).apiClient.adminAccountDeletionRequests();
    if (!mounted) return;
    if (result.ok) setState(() => _deletionRequests = _listFrom(result.data?['requests']));
  }

  Future<void> _loadAuditLogs() async {
    final result = await AppScope.of(context).apiClient.adminAuditLogs();
    if (!mounted) return;
  ...
  Future<void> _setStatus(Map<String, dynamic> user, String status) async {
    ...
    final result = await AppScope.of(context).apiClient.adminSetUserStatus(id, status, reason: reason);
    if (!mounted) return;
    if (!result.ok) {
  ...
  Future<void> _editUser(Map<String, dynamic> user) async {
    ...
    username.dispose();
    if (body == null) return;
    final result = await AppScope.of(context).apiClient.adminUpdateUser(id, body);
    if (!mounted) return;
    if (!result.ok) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(result.error ?? context.tr('connectionProblem'))));
      return;
  ...
  Future<void> _openInbox(Map<String, dynamic> user) async {
    ...
    if (reason == null) return;
    final result = await AppScope.of(context).apiClient.adminUserChats(id, reason);
    if (!mounted) return;
    if (!result.ok) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(result.error ?? context.tr('connectionProblem'))));
      return;
  ...
  Future<void> _openConversationMessages(int conversationId, String reason) async {
    ...
    final result = await AppScope.of(context).apiClient.adminPrivateMessages(conversationId, reason);
    if (!mounted) return;
    if (!result.ok) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(result.error ?? context.tr('connectionProblem'))));
      return;
  ...
  Future<void> _deletePrivateMessage(int messageId) async {
    ...
    if (reason == null) return;
    final result = await AppScope.of(context).apiClient.adminDeletePrivateMessage(messageId, reason);
    if (!mounted) return;
  ...
  Future<void> _reviewDeletion(Map<String, dynamic> request, String action) async {
    ...
    if (reason == null) return;
    final result = await AppScope.of(context).apiClient.adminReviewAccountDeletion(id, action, notes: reason);
    if (!mounted) return;
    if (!result.ok) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(result.error ?? context.tr('connectionProblem'))));
      return;
