...
class ApiClient {
  ...
  Future<ApiResult<Map<String, dynamic>>> studentProfile() async {
    try {
      final res = await http
          .get(await _uri('/api/v1/student/profile'), headers: await _headers(auth: true))
          .timeout(const Duration(seconds: 10));
      return _decodeMap(res);
    } catch (e) {
      return ApiResult.failure('Could not load student profile: $e');
    }
  }

  Future<ApiResult<Map<String, dynamic>>> subscription() async {
    try {
      final res = await http
          .get(await _uri('/api/v1/student/subscription'), headers: await _headers(auth: true))
          .timeout(const Duration(seconds: 10));
      return _decodeMap(res);
    } catch (e) {
      return ApiResult.failure('Could not load subscription: $e');
    }
  }
